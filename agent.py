...
class Agent():
  ...
  def act(self, state):
    with torch.no_grad():
      # (self.online_net(state.unsqueeze(0)) * self.support).sum(2).argmax(1).item()
      # value1 = (self.online_net(state.unsqueeze(0)) * self.support)
      # value2 = sum(2).argmax(1).item()
  ...
  def learn(self, mem):
    ...
    with torch.no_grad():
      ...
      pns = self.target_net(next_states)  # Probabilities p(s_t+n, ·; θtarget) || shape : torch.Size([64, 3, 51])

      # print(range(self.batch_size))
      # print(argmax_indices_ns)
      # pns[range(32), 64]
